...
export class Crudder<T extends { [key in K]: unknown }, K extends keyof T> {
  ...
  // Calls update
  public updateMany(items: { data: Partial<Omit<T, K>>; key: T[K] }[]) {
    const results = [];
    for (const item of items) {
      results.push(this.update(item.key, item.data));
    }
  ...
  public upsert(key: T[K], data: Omit<T, K>) {
    ...
    return this.create({ ...data, [this.key]: key } as T);
  }
}
